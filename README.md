...
- **メール送信**：Microsoft Graph API（Entraアプリ登録）による認証付き送信。企業ごとにメール1をTo、メール2以降をCCとして1通に集約し、事務局CC・BCC、テストモード、全宛先の送信直前確認、企業横断の重複アドレス検出に対応。役職/委員会/会議出欠による宛先絞り込み、差し込みデータ（`{事業所名}` `{氏名}` `{col1}`〜`{col5}` 等）、共通・会社別添付ファイルも利用可能
