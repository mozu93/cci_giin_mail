...
- **メール送信**：Microsoft Graph API（Entraアプリ登録）による認証付き送信、CC/BCC、全件を安全な宛先へ振り替えるテストモード、認証アカウント・重複宛先の安全確認、役職/委員会/会議出欠による宛先絞り込み、差し込みデータ（`{事業所名}` `{氏名}` `{col1}`〜`{col5}` 等）、共通・個別添付ファイル、履歴付きテスト送信
